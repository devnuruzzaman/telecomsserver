            ...
            },
            {
              ...
              children: [
                ...
                },
                {
                  ...
                  blueprintContent: `import { Processor, WorkerHost } from '@nestjs/bullmq';\nimport { Job } from 'bullmq';\nimport { HttpService } from '@nestjs/axios';\nimport { firstValueFrom } from 'rxjs';\nimport { PrismaService } from '../prisma/prisma.service';\n\n@Processor('recharges')\nexport class RechargeProcessor extends WorkerHost {\n  constructor(\n    private prisma: PrismaService,\n    private httpService: HttpService\n  ) {\n    super();\n  }\n\n  async process(job: Job<any>): Promise<any> {\n    const { rechargeId, recipientNumber, amount, operatorCode } = job.data;\n    try {\n      // Execute HTTP POST Handshake to direct telecom provider API\n      const gatewayUrl = process.env.TELECOM_GATEWAY_URL;\n      const response = await firstValueFrom(\n        this.httpService.post(gatewayUrl, {\n          msisdn: recipientNumber,\n          denomination: amount,\n          operator: operatorCode,\n          partnerId: process.env.PARTNER_ID\n        }, {\n          headers: { 'Authorization': \`Bearer \${process.env.GATEWAY_TOKEN}\` }\n        })\n      );\n\n      await this.prisma.recharge.update({\n        where: { id: rechargeId },\n        data: {\n          status: 'SUCCESS',\n          gatewayReference: response.data.transactionId,\n          gatewayResponse: response.data,\n          completedAt: new Date()\n        }\n      });\n\n      return { success: true, txnId: response.data.transactionId };\n    } catch (error) {\n      // Failed operator connection triggers self-healing failover mechanism\n      await this.prisma.recharge.update({\n        where: { id: rechargeId },\n        data: {\n          status: 'FAILED',\n          gatewayResponse: { error: error.message, stack: error.stack }\n        }\n      });\n      throw error;\n    }\n  }\n}`
                },
                {
                  ...
                  blueprintContent: `import { Injectable, BadRequestException } from '@nestjs/common';\nimport { InjectQueue } from '@nestjs/bullmq';\nimport { Queue } from 'bullmq';\nimport { PrismaService } from '../prisma/prisma.service';\nimport { WalletService } from '../wallet/wallet.service';\n\n@Injectable()\nexport class RechargeService {\n  constructor(\n    private prisma: PrismaService,\n    private walletService: WalletService,\n    @InjectQueue('recharges') private rechargeQueue: Queue\n  ) {}\n\n  async initiateRecharge(dto: any) {\n    const { userId, recipientNumber, amount, operatorId, packageId } = dto;\n    const operator = await this.prisma.operator.findUnique({ where: { id: operatorId } });\n    if (!operator || !operator.isActive) {\n      throw new BadRequestException('Target operator is currently offline');\n    }\n\n    // debitWallet handles atomic SELECT FOR UPDATE ledger debit locks\n    const recharge = await this.prisma.$transaction(async (tx) => {\n      const dbRecharge = await tx.recharge.create({\n        data: {\n          userId,\n          operatorId,\n          packageId,\n          recipientNumber,\n          amount,\n          status: 'INITIATED'\n        }\n      });\n      await this.walletService.debitWallet(userId, amount, dbRecharge.id);\n      return dbRecharge;\n    });\n\n    // Dispatch non-blocking worker pipeline processing via BullMQ redis backend\n    await this.rechargeQueue.add('execute-recharge', {\n      rechargeId: recharge.id,\n      recipientNumber,\n      amount,\n      operatorCode: operator.code\n    });\n\n    return {\n      success: true,\n      message: 'Recharge queued in telecom gateway pipeline',\n      rechargeId: recharge.id\n    };\n  }\n}`
                }
              ]
            },
            {
